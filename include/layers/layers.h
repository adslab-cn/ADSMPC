...
public:
    ...
    bool isFirst = false;
    u64 scale = 0;
    int mode = 0; // only used in ReLU in FSS improved to decide between relu and reluext, might need something cleaner?
...
{
public:
    Tensor<T> inp; // X: 特征矩阵 (num_nodes, in)
    ...
    void _forward(Tensor<T> &x)
    {
        std::cout << "forward_GCNconv" << std::endl;

        auto x_2d = x.as_2d(); // 转换输入x的形状为2D
        Tensor2D<T> AX(A.d1, x.d2);  // 用于存储 A * X(a) 的结果
        this->backend->matmul(A, x_2d, AX); // 先执行矩阵乘法：A * X
        std::cout << "AX OK " << std::endl;

        auto act_2d = this->activation.as_2d(); // 这一层的最终输出矩阵
        this->backend->matmul(AX, weight, act_2d, this->useBias, bias, this->isFirst);
        this->activation.d_data = act_2d.d_data;
    }
