...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "ElemWiseMul", keyread_time, compute_time, reconstruction_stats.first, reconstruction_stats.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "EdabitsPrTrunc", keyread_time, compute_time, reconstruction_stats.first, reconstruction_stats.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "LUTSS", keyread_time, compute_time_1 + compute_time_2, reconstruction_stats_1.first + reconstruction_stats_2.first, reconstruction_stats_1.second + reconstruction_stats_2.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "LUT", keyread_time, compute_time, reconstruction_stats.first, reconstruction_stats.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "LUT::FirstRound", keyread_time, compute_time_1, reconstruction_stats_1.first, reconstruction_stats_1.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
#pragma omp parallel for
    ...
    if (party != DEALER)
    {
        FSS::stat_t stat = {prefix + "LUT::Misc", 0, t, 0, 0, 0};
        stat.print();
        FSS::push_stats(stat);
    }
...
#pragma omp parallel for
    ...
    LUT_dpf(size, 8, FSSConfig::bitlength, lut_0, x0, x0, "nExp::");
    LUT_dpf(size, 8, FSSConfig::bitlength, lut_1, x1, x1, "nExp::");
    ...
    if (party != DEALER)
    {
        FSS::push_stats({"nExp::Misc", 0, t1, 0, 0, 0});
    }
...
void nExp(int size, int bin, GroupElement *x, GroupElement *y, int scale)
{
    ...
    LUT_dpf(size, lutBw, FSSConfig::bitlength, lut, y, y, "nExp::");
}

void Tanh(int size, GroupElement *x, GroupElement *y, int scale)
{
    int lutBw = 14;
    always_assert(FSSConfig::bitlength >= lutBw);

    GroupElement *abs = new GroupElement[size];
    GroupElement *drelu = new GroupElement[size];
    // Relu2Round(size, x, x, abs, abs, drelu, FSSConfig::bitlength);
...
#pragma omp parallel for
    ...
    LUT_dpf(size, lutBw, FSSConfig::bitlength, lut, abs, abs, "Tanh::");
...
void InverseLUT(int size, GroupElement *x, GroupElement *y, int scale, int bw, std::string prefix = "")
{
    ...
    LUT_dpf(size, 16, FSSConfig::bitlength, lut, y, y, prefix + "Inverse::");
}
...
#pragma omp parallel for
    ...
    if (party != DEALER)
        FSS::push_stats({"Softmax::Misc", 0, t1 + t2 + t3, 0, 0, 0});
...
#pragma omp parallel for
    ...
    // if (party != DEALER)
    // {
    //     // FSS::stat_t stat = { "Rsqrt::LutGen", 0, t, 0, 0, 0 };
    //     // stat.print();
    //     // FSS::push_stats(stat);
    // }
...
#pragma omp parallel for
    ...
    if (party != DEALER)
    {
        FSS::push_stats({"GeLU::Misc", 0, t1 + t3, 0, 0, 0});
        // FSS::push_stats({ "GeLU::LutGen", 0, t2, 0, 0, 0 });
        auto reconstruction_stats = time_comm_this_block([&]()
...
#pragma omp parallel for
    ...
    if (party != DEALER)
    {
        FSS::push_stats({"SiLU::Misc", 0, t1 + t3, 0, 0, 0});
        // FSS::push_stats({ "GeLU::LutGen", 0, t2, 0, 0, 0 });
        auto reconstruction_stats = time_comm_this_block([&]()
                                                         { reconstruct(size, out, bitlength); });
...
#pragma omp parallel for
    ...
    delete[] drelu;
    if (party != DEALER)
        FSS::push_stats({prefix + "Clip::Misc", 0, t1 + t2 + t3, 0, 0, 0});
}
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "SumOfSquare", keyread_time, compute_time, reconstruction_stats.first, reconstruction_stats.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {prefix + "Div::Misc", 0, t, 0, 0, 0};
        stat.print();
        FSS::push_stats(stat);
...
void SlothDivFaithful(int size, GroupElement *x, GroupElement *y, GroupElement divisor, int scale, std::string prefix = "")
{
    if (!(divisor & (divisor - 1)))
    {
        SlothFaithfulARS(size, FSSConfig::bitlength, x, y, log2(divisor), prefix + "Div::");
    }
    else
    {
        GroupElement divfp = (1LL << scale) / divisor;
...
#pragma omp parallel for
        ...
        SlothFaithfulARS(size, FSSConfig::bitlength, y, y, scale, prefix + "Div::");
    }
}
...
#pragma omp parallel for collapse(2)
    ...
    FSS::stat_t stat = {"LayerNorm::Misc", 0, t1 + t2 + t3 + t4 + t5, 0, 0, 0};
    stat.print();
    FSS::push_stats(stat);
...
// void SlothRMSNorm(int s1, int s2, GroupElement *x, GroupElement *A, GroupElement *y, int scale)
{
    ...
    FSS::stat_t stat = {"LayerNorm::Misc", 0, t1 + t2 + t3, 0, 0, 0};
    stat.print();
    FSS::push_stats(stat);
...
#pragma omp parallel for
        ...
        FSS::stat_t stat = {"Gemm", keyread_time_1 + keyread_time_2, compute_time_1 + compute_time_2, reconstruction_stats_1.first + reconstruction_stats_2.first + msb_stats.first, reconstruction_stats_1.second + reconstruction_stats_2.second + msb_stats.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
void SoftmaxTriangular(int32_t s1, int32_t s2, int bin, GroupElement *x, GroupElement *y, int32_t scale)
{
    ...
    if (party != DEALER)
        FSS::push_stats(FSS::stat_t{"Softmax::Misc", 0, t1 + t2 + t3 + t4, 0, 0, 0});
                        ...
                        MASK_PAIR(GroupElement *B), MASK_PAIR(GroupElement *C), bool modelIsA)
{
    ...
    else
    {
        ...
        FSS::stat_t stat = {"Linear::MatMul", keyread_time, compute_time + t1, reconstruction_stats.first, reconstruction_stats.second, dealer->bytesReceived() - keysize_start};
        stat.print();
        FSS::push_stats(stat);
...
void GTDCFReLU(int32_t size, GroupElement *inArr, GroupElement *outArr, GroupElement *inArr_mask, GroupElement *outArr_mask, int suffix_w, std::string prefix)
{
    ...
    if (party == DEALER) {
        auto keys = new std::pair<GTDCFKeyPack, GTDCFKeyPack>[size];
        auto start = std::chrono::high_resolution_clock::now();
        
        #pragma omp parallel for
        for(int i=0; i<size; ++i) {
            GroupElement rin = inArr_mask[i]; // Dealer知道阈值 r_in
            GroupElement rout = random_ge(bin);
            if(outArr_mask) outArr_mask[i] = rout;
        ...
        auto keys = new GTDCFKeyPack[size];
        uint64_t keysize_start = dealer->bytesReceived();
        ...
        // 在该框架中，inArr 已经是公共掩码值 z = x + r_in！
        // 直接执行稀疏查树，跳过前置的 reconstruct!
        auto t_eval = time_this_block([&](){
            #pragma omp parallel for
            for(int i=0; i<size; ++i) {
                GroupElement res[2];
                evalGTDCF(party - SERVER, keys[i], inArr[i], res);
                
                GroupElement q_share = res[0]; 
                GroupElement z_share = res[1]; 

                // [y] = z * [q] + [z] + [rout]
                outArr[i] = inArr[i] * q_share + z_share + keys[i].rout_share;
                mod(outArr[i], bin);
        ...
        // 最后，将各方计算出的输出份额 [y+rout] 重构合并，作为公共输出给下一层网络
        auto comm = time_comm_this_block([&](){
            reconstruct(size, outArr, bin);
        });
        
        evalMicroseconds += (t_eval + comm.first);
        
        FSS::stat_t stat = {
            prefix + "GTDCF-ReLU", keyread_time, t_eval, comm.first, comm.second, dealer->bytesReceived() - keysize_start
        };
        stat.print();
        FSS::push_stats(stat);
