...
int main(int argc, char** argv) {
    ...
    const double SHUF_OFF_COMM_100K = 17.6;  // MB
    int num_layers = 2;
    ...
    uint64_t base_c_start = get_online_comm();
    ...
    uint64_t base_c_end = get_online_comm();
    FSS::end();
    
    double base_mat_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(base_end - base_start).count();
    double base_mat_comm_MB = (base_c_end - base_c_start) / (1024.0 * 1024.0);

    // =======================================================
    // 执行 Queries
    // =======================================================
    int numQueries = 3; 
    int numDeltaNodes = 10;   // Cora 10% 规模
    int numDeltaEdges = 10;
    int numGhostNodes = 5;
    ...
    for (int q = 1; q <= numQueries; q++) {
        ...
        uint64_t c_query_start = get_online_comm();

        // 1) L1 降维 (增量计算)
        auto t1_start = std::chrono::high_resolution_clock::now();
        uint64_t c1_s = get_online_comm();
        ...
        uint64_t c1_e = get_online_comm();

        // 2) L1 消息传递
        auto t2_start = std::chrono::high_resolution_clock::now();
        uint64_t c2_s = get_online_comm();
        ...
        uint64_t c4_e = get_online_comm();

        // 5) L2 消息传递
        auto t5_start = std::chrono::high_resolution_clock::now();
        uint64_t c5_s = get_online_comm();
        BPMPL_GraphRouting(numBaseNodes, numBaseEdges, numDeltaNodes, numDeltaEdges, numGhostNodes,
        ...
        uint64_t c5_e = get_online_comm();

        // 6) Softmax
        auto t6_start = std::chrono::high_resolution_clock::now();
        uint64_t c6_s = get_online_comm();
        int fixed_point_scale = 12;
        BPGCNSoftmax(totalNodes, outDim, H2_pre, Y_out, H2_pre_mask, MaskForY_out, fixed_point_scale, "Out_Softmax");
        auto t6_end = std::chrono::high_resolution_clock::now();
        uint64_t c6_e = get_online_comm();

        auto query_end = std::chrono::high_resolution_clock::now();
        FSS::end();

        // =======================================================
        // 打印：静态与动态场景的开销账单 (时间 + 通信量)
        // =======================================================
        if (party == CLIENT) {
            ...
            double m1_delta = (c1_e - c1_s) / (1024.0 * 1024.0);
            double m2 = (c2_e - c2_s) / (1024.0 * 1024.0);
            double m3 = (c3_e - c3_s) / (1024.0 * 1024.0);
            double m4 = (c4_e - c4_s) / (1024.0 * 1024.0);
            double m5 = (c5_e - c5_s) / (1024.0 * 1024.0);
            double m6 = (c6_e - c6_s) / (1024.0 * 1024.0);
            ...
            double mono_off_comm = (N_tot * total_shuffles / 100000.0) * SHUF_OFF_COMM_100K;
            ...
            double bpgnn_off_comm = (N_delta * total_shuffles / 100000.0) * SHUF_OFF_COMM_100K;

            double offline_prep_time, offline_prep_comm;
            double l1_trans_time, l1_trans_comm;
            double total_time, total_comm;

            std::cout << "\n========================================================================" << std::endl;
            if (q == 1) {
                std::cout << ">>>[Query " << q << ": Static / Transductive Setting] (Full Graph Processing)" << std::endl;
                offline_prep_time = mono_off_time;                  
                offline_prep_comm = mono_off_comm;                  
                l1_trans_time = base_mat_time_ms + d1_delta;        
                l1_trans_comm = base_mat_comm_MB + m1_delta;        
            } else {
                std::cout << ">>> [Query " << q << ": Dynamic / Inductive Setting] (Incremental Processing)" << std::endl;
                offline_prep_time = bpgnn_off_time;                 
                offline_prep_comm = bpgnn_off_comm;                 
                l1_trans_time = d1_delta;                           
                l1_trans_comm = m1_delta;                           
            }

            total_time = offline_prep_time + l1_trans_time + d2 + d3 + d4 + d5 + d6;
            total_comm = offline_prep_comm + l1_trans_comm + m2 + m3 + m4 + m5 + m6;
            ...
            std::cout << "  ----------------------------------------------------------------------" << std::endl;
            if (q == 1) {
                std::cout << "  => Total Static Inference Time            : " << total_time << " ms \t| Total Comm: " << total_comm << " MB" << std::endl;
            } else {
                std::cout << "  => Total Dynamic Inference Time           : " << total_time << " ms \t| Total Comm: " << total_comm << " MB" << std::endl;
            }
            std::cout << "========================================================================\n" << std::endl;
        }
        ...
        delete [] D_inv;
        delete[] D_inv_mask;
        delete [] ghostIndices_mask;
        
        delete[] H_delta_trans;
        delete[] H_delta_trans_mask;
        delete [] H_trans_total;
        delete[] H_trans_total_mask;
        ...
        delete[] H2_pre_mask;
        delete [] Y_out;
        GroupElement* safe_ptr = MaskForY_out;
        delete[] safe_ptr;
    }
