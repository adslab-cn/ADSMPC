...
inline void reconstrunct_and_print_float(Tensor4D<u64> &in, int FSSParty) {
...
inline void softmax_secfloat(Tensor4D<u64> &in, Tensor4D<u64> &out, u64 scale, int FSSParty)
{
    assert(in.d1 == out.d1);
    assert(in.d2 == out.d2);
    assert(in.d3 == 1);
    ...
    int origBitlength = FSSConfig::bitlength;
    FSSConfig::bitlength = origBitlength - scale;
    FixToFloat(in.d1 * in.d2, in.data, inFloat.data, scale);
    FSSConfig::bitlength = origBitlength;
    Tensor4D<u64> outFloat(in.d1, in.d2, 4, 1);
    outFloat.fill(0);
    if (FSSParty != 1) {
        vector < vector < FPArray > > inpFloatSecfloat = make_vector_float(FSSParty-1, in.d1, in.d2);
        for(int i = 0; i < in.d1; ++i) {
            for(int j = 0; j < in.d2; ++j) {
        ...
        vector < vector < FPArray > > outFloatSecfloat = make_vector_float(FSSParty-1, in.d1, in.d2);
        ...
        int sz = in.d1 * in.d2;
        vector < FPArray > outFloatSecfloatFlat = make_vector_float(FSSParty-1, sz);
        for(int i = 0; i < in.d1; ++i) {
            for(int j = 0; j < in.d2; ++j) {
        ...
        vector<FPArray> divver = make_vector_float(FSSParty-1, sz) ;
        for (int i = 0 ; i < sz ; i++)
