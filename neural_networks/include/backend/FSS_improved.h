...
// in FSSImproved, mode takes the value according to the following rule:
...
template <typename T>
class FSSImproved : public FSSBase<T> {
public:
    ...
    void relu(const Tensor4D<T> &in, const Tensor4D<T> &out, const Tensor4D<T> &drelu, u64 scale, int mode) {
        ...
        if (mode == 0) {
            Relu2Round(sz, in.data, in.data, out.data, out.data, drelu.data, FSSConfig::bitlength);
        }
        else if (mode == 3) {
            for(int i = 0; i < sz; i++) {
                in.data[i] = in.data[i] % (1ULL << (FSSConfig::bitlength - scale));
            }
            FSSConfig::bitlength -= scale;
            Relu2Round(sz, in.data, in.data, out.data, out.data, drelu.data, FSSConfig::bitlength);
            FSSConfig::bitlength += scale;
        }
        else if (mode == 2) {
            ...
            ReluExtend(sz, FSSConfig::bitlength - scale, FSSConfig::bitlength, in.data, out.data, drelu.data);
        }
        else {
    ...
    void truncate(T *in, T *out, u64 shift, u64 size, u8 mode) {
        ...
        if (mode == 0) {
            SignExtend2(size, FSSConfig::bitlength - shift, FSSConfig::bitlength, out, out);
        }
        else if (mode == 1) {
        }
        else {
    ...
    void maxPool2D(u64 ks, u64 padding, u64 stride, const Tensor4D<T> &in, Tensor4D<T> &out, Tensor4D<u64> &maxIdx, u64 scale, u8 mode) {
        ...
        if (mode == 0) {
            ...
            FSSConfig::bitlength -= scale; // why is this hack not working? new intel - it works
            MaxPoolDouble(out.d1, out.d2, out.d3, out.d4, ks, ks, padding, padding, padding, padding, stride, stride, in.d1, in.d2, in.d3, in.d4, in.data, in.data, out.data, out.data, maxBit.data);
            FSSConfig::bitlength += scale;
        }
        else if (mode == 2) {
    ...
    {
        SignExtend2(x.d1 * x.d2 * x.d3 * x.d4, FSSConfig::bitlength - scale, FSSConfig::bitlength, x.data, x.data);
    }
