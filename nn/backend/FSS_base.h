#pragma once
#include "../utils.h"
#include "../../crypto/FSS/aux_parameter/config.h"
#include "../../crypto/FSS/aux_parameter/input_prng.h"
#include "../../crypto/FSS/aux_parameter/comms.h"
#include "../../crypto/FSS/api/api.h"
#include "backend.h"
#include "../layers/layers.h"
#include <filesystem>

template <typename T>
class Sequential;

char *readFile(std::string filename)
{
    std::ifstream file(filename, std::ios::binary);
    size_t fileSz = std::filesystem::file_size(filename);
    char *mem_bytes = (char *)malloc(fileSz);
    file.read((char *)mem_bytes, fileSz);
    file.close();
    return mem_bytes;
}

template <typename T>
class FSSBase : public Backend<T>
{
public:
    const bool useLocalTruncation = false;
    char *_kBuf;

    void initPrngs()
    {
        u64 seedKey = 0xdeadbeefbadc0ffe;
        for (int i = 0; i < 256; ++i)
        {
            FSSConfig::prngs[i].SetSeed(osuCrypto::toBlock(i, seedKey));
        }
    }

    void initDealer(char **sBuf, char **cBuf)
    {
        assert(FSSConfig::party == 1);
        initPrngs();
        FSSConfig::server = new Peer(sBuf);
        FSSConfig::client = new Peer(cBuf);
        // input_prng_init();
    }

    void initServer(std::string ip, char **kBuf)
    {
        assert(FSSConfig::party == 2);
        initPrngs();
        FSSConfig::dealer = new Dealer(kBuf);
        FSSConfig::client = waitForPeer(42002);
        FSSConfig::peer = FSSConfig::client;
        // input_prng_init();
    }

    void initClient(std::string ip, char **kBuf)
    {
        assert(FSSConfig::party == 3);
        initPrngs();
        FSSConfig::dealer = new Dealer(kBuf);
        FSSConfig::server = new Peer(ip, 42002);
        FSSConfig::peer = FSSConfig::server;
        // input_prng_init();
    }

    void init(std::string ip, bool memBuf = false)
    {
        if (FSSConfig::party == 1)
        {
            initPrngs();
            FSSConfig::server = new Peer("server.dat");
            FSSConfig::client = new Peer("client.dat");
        }
        else if (FSSConfig::party == 2)
        {
            if (memBuf)
            {
                _kBuf = readFile("server.dat");
                initServer(ip, &_kBuf);
            }
            else
            {
                initPrngs();
                FSSConfig::dealer = new Dealer("server.dat");
                FSSConfig::client = waitForPeer(42005);
                FSSConfig::peer = FSSConfig::client;
            }
        }
        else if (FSSConfig::party == 3)
        {
            if (memBuf)
            {
                _kBuf = readFile("client.dat");
                initClient(ip, &_kBuf);
            }
            else
            {
                initPrngs();
                FSSConfig::dealer = new Dealer("client.dat");
                FSSConfig::server = new Peer(ip, 42005);
                FSSConfig::peer = FSSConfig::server;
            }
        }
        else
        {
            throw std::runtime_error("Invalid party");
        }
        input_prng_init();
    }

    void finalize()
    {
        switch (FSSConfig::party)
		{
		case 1:
				FSSConfig::server->close();
				FSSConfig::client->close();
				break;
		case 2:
				FSSConfig::dealer->close();
				FSSConfig::client->close();
				break;
		case 3:
				FSSConfig::dealer->close();
				FSSConfig::server->close();
		}
    }

    void initializeData(Tensor4D<T> &data, u64 numImagesWithServer)
    {
        u64 b1 = numImagesWithServer * data.d2 * data.d3 * data.d4;
        u64 b2 = (data.d1 - numImagesWithServer) * data.d2 * data.d3 * data.d4;
        if (FSSConfig::party == 1) {
            input_layer(nullptr, data.data, b1, 2);
            input_layer(nullptr, data.data + b1, b2, 3);
        }
        else {
            if (FSSConfig::party == 2) {
                Tensor4D<T> tmp(numImagesWithServer, data.d2, data.d3, data.d4);
                input_layer(data.data, tmp.data, b1, 2);
                input_layer(data.data + b1, nullptr, b2, 3);
            }
            else {
                Tensor4D<T> tmp(data.d1 - numImagesWithServer, data.d2, data.d3, data.d4);
                input_layer(data.data, nullptr, b1, 2);
                input_layer(data.data + b1, tmp.data, b2, 3);
            }
        }
    }

    void initializeInferencePartyB(Tensor4D<T>&data){
        u64 size = data.d1 * data.d2 * data.d3 * data.d4;
        if(FSSConfig::party == 1){
            input_layer(nullptr,data.data, size,3);
        }
        else{
            Tensor4D<T> tmp(data.d1, data.d2, data.d3, data.d4);
            input_layer(data.data, tmp.data, size, 3);
        }
    }

    void initializeInferencePartyA(LayerGraphNode<T> *root){
         topologicalApply(root, [&](LayerGraphNode<T> *node, LayerGraphNode<T> *_root) {
            auto layer = node->layer;
            if(layer->name.find("Conv2D") != std::string::npos || layer->name.find("FC") != std::string::npos) {
                auto& weights = layer->getweights();
                auto& bias = layer->getbias();
                if(FSSConfig::party == 1){
                    input_layer(nullptr,weights.data, weights.d1*weights.d2,2);
                    if (layer->useBias) {
                        input_layer(nullptr,bias.data, bias.size,2);
                    }
                }
                else{
                    Tensor2D<T> tmp(weights.d1, weights.d2);
                    input_layer(weights.data, tmp.data, weights.d1*weights.d2, 2);
                    if(layer->useBias){
                        Tensor<T> tmp2(bias.size);
                        input_layer(bias.data, tmp2.data, bias.size, 2);
                    }
                }
            }
            // else if (layer->name.find("BatchNorm2dInference") != std::string::npos) {
            //     auto bn = (BatchNorm2dInference<T>*) layer;
            //     auto channel = bn->A.size;
            //     if(FSSConfig::party == 1){
            //         input_layer(nullptr,bn->A.data, channel,2);
            //         input_layer(nullptr,bn->B.data, channel,2);
            //     }
            //     else{
            //         Tensor<T> tmp(channel);
            //         input_layer(bn->A.data, tmp.data, channel, 2);
            //         input_layer(bn->B.data, tmp.data, channel, 2);
            //     }
            // }
        });
    }

    void inputA(Tensor4D<T> &data)
    {
        u64 b1 = data.d1 * data.d2 * data.d3 * data.d4;
        if (FSSConfig::party == 1) {
            input_layer(nullptr, data.data, b1, 2);
        }
        else {
            if (FSSConfig::party == 2) {
                Tensor4D<T> tmp(data.d1, data.d2, data.d3, data.d4);
                input_layer(data.data, tmp.data, b1, 2);
            }
            else {
                input_layer(data.data, nullptr, b1, 2);
            }
        }
    }

    // 初始化权重参数
    void initializeWeights(Sequential<T> &model)
    {
        // DEALER selects the inital weights and sends them to parties as keys
        for(int i = 0; i < model.layers.size(); ++i)
        {
            if (model.layers[i]->name == "Conv2D" || model.layers[i]->name == "FC" || model.layers[i]->name == "GCNConv")
            {
                auto &weights = model.layers[i]->getweights();
                auto &bias = model.layers[i]->getbias();
                if (FSSConfig::party == 1)
                {
                    // weights.fill(1);
                    // bias.fill(1);
                    FSSConfig::server->send_ge_array(weights.data, weights.d1 * weights.d2);
                    FSSConfig::server->send_ge_array(bias.data, bias.size);
                    FSSConfig::client->send_ge_array(weights.data, weights.d1 * weights.d2);
                    FSSConfig::client->send_ge_array(bias.data, bias.size);
                    weights.fill(0);
                    bias.fill(0);
                }
                else
                {
                    FSSConfig::dealer->recv_ge_array(weights.data, weights.d1 * weights.d2);
                    FSSConfig::dealer->recv_ge_array(bias.data, bias.size);
                }
            }
        }
    }

     void outputA(Tensor4D<T> &a) {
        u64 sz = a.d1 * a.d2 * a.d3 * a.d4;
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a.data[i]);
                a.data[i] = 0;
            }
        }
        else if(FSSConfig::party ==3){
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a.data[i] = a.data[i] - mask;
            }
        }
    }

    void output(Tensor4D<T> &a) {
        u64 sz = a.d1 * a.d2 * a.d3 * a.d4;
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a.data[i]);
                FSSConfig::server->send_mask(a.data[i]);
                a.data[i] = 0;
            }
        }
        else {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                if(i==33)
                {
                    std::cerr<<"mask: "<<mask<<std::endl;
                }
                a.data[i] = a.data[i] - mask;
            }
        }
    }

    void outputA(Tensor2D<T> &a) {
        u64 sz = a.d1 * a.d2;
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a.data[i]);
                a.data[i] = 0;
            }
        }
        else if(FSSConfig::party ==3) {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a.data[i] = a.data[i] - mask;
            }
        }
    }

    void output(Tensor2D<T> &a) {
        u64 sz = a.d1 * a.d2;  //计算张量大小
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a.data[i]);
                FSSConfig::server->send_mask(a.data[i]);
                a.data[i] = 0;
            }
        }
        else {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a.data[i] = a.data[i] - mask;
            }
        }
    }

    void outputA(Tensor<T> &a) {
        u64 sz = a.size;
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a.data[i]);
                a.data[i] = 0;
            }
        }
        else if(FSSConfig::party ==3) {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a.data[i] = a.data[i] - mask;
            }
        }
    }

    void output(Tensor<T> &a) {
        u64 sz = a.size;
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a.data[i]);
                FSSConfig::server->send_mask(a.data[i]);
                a.data[i] = 0;
            }
        }
        else {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a.data[i] = a.data[i] - mask;
            }
        }
    }

    void outputA(T *a, u64 sz) {
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a[i]);
                a[i] = 0;
            }
        }
        else if(FSSConfig::party ==3) {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a[i] = a[i] - mask;
            }
        }
    }

    void output(T *a, u64 sz) {
        if (FSSConfig::party == 1) {
            for (int i = 0; i < sz; i++){
                FSSConfig::client->send_mask(a[i]);
                FSSConfig::server->send_mask(a[i]);
                a[i] = 0;
            }
        }
        else {
            for (int i = 0; i < sz; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                a[i] = a[i] - mask;
            }
        }
    }

    void ss2m(T *data, u64 size)
    {
        std::cerr << ">> SS2M - Start" << std::endl;
        if (FSSConfig::party == 1) {
            for (int i = 0; i < size; i++){
                data[i] = random_ge(64);
                auto p = splitShare(data[i], 64);
                FSSConfig::client->send_mask(p.first);
                FSSConfig::server->send_mask(p.second);
            }
        }
        else {
            for (int i = 0; i < size; i++){
                auto mask = FSSConfig::dealer->recv_mask();
                data[i] = data[i] + mask;
            }
            reconstruct(size, data, 64);
        }
        std::cerr << ">> SS2M - End" << std::endl;
    }

    void matmul(const Tensor2D<T> &a, const Tensor2D<T> &b, Tensor2D<T> &c) {
        assert(a.d2 == b.d1);
        assert(c.d1 == a.d1);
        assert(c.d2 == b.d2);
        MatMul2D(a.d1, a.d2, b.d2, a.data, a.data, b.data, b.data, c.data, c.data, true);
    }

    void matmul(const Tensor4D<T> &a, const Tensor2D<T> &b, Tensor4D<T> &c) {
        assert(a.d2 == b.d1);
        assert(a.d3 == 1);
        assert(a.d4 == 1);
        assert(c.d1 == a.d1);
        assert(c.d2 == b.d2);
        assert(c.d3 == 1);
        assert(c.d4 == 1);

        MatMul2D(a.d1, a.d2, b.d2, a.data, a.data, b.data, b.data, c.data, c.data, true);
    }

    void matmul(const Tensor2D<T> &a, const Tensor4D<T> &b, Tensor4D<T> &c) {
        assert(a.d2 == b.d1);
        assert(b.d3 == 1);
        assert(b.d4 == 1);
        assert(c.d1 == a.d1);
        assert(c.d2 == b.d2);
        assert(c.d3 == 1);
        assert(c.d4 == 1);

        MatMul2D(a.d1, a.d2, b.d2, a.data, a.data, b.data, b.data, c.data, c.data, true);
    }

    void matmulTransposeA(const Tensor4D<T> &a, const Tensor4D<T> &b, Tensor2D<T> &c) {
        assert(a.d1 == b.d1);
        assert(a.d3 == 1);
        assert(a.d4 == 1);
        assert(b.d3 == 1);
        assert(b.d4 == 1);
        assert(c.d1 == a.d2);
        assert(c.d2 == b.d2);

        Tensor2D<T> aTranspose(a.d2, a.d1);
        for(int i = 0; i < a.d1; ++i)
            for(int j = 0; j < a.d2; ++j)
                aTranspose(j, i) = a(i, j, 0, 0);
        MatMul2D(aTranspose.d1, aTranspose.d2, b.d2, aTranspose.data, aTranspose.data, b.data, b.data, c.data, c.data, true);
    }

    void matmulTransposeB(const Tensor4D<T> &a, const Tensor2D<T> &b, Tensor4D<T> &c) {
        assert(a.d2 == b.d2);
        assert(a.d3 == 1);
        assert(a.d4 == 1);
        assert(c.d1 == a.d1);
        assert(c.d2 == b.d1);
        assert(c.d3 == 1);
        assert(c.d4 == 1);
        Tensor2D<T> bTranspose(b.d2, b.d1);
        for(int i = 0; i < b.d1; ++i)
            for(int j = 0; j < b.d2; ++j)
                bTranspose(j, i) = b(i, j);
        matmul(a, bTranspose, c);
    }

    // void matmulTransposeB(const Tensor2D<T> &a, const Tensor4D<T> &b, Tensor4D<T> &c) {
    //     assert(a.d2 == b.d2);
    //     assert(b.d3 == 1);
    //     assert(b.d4 == 1);
    //     assert(c.d1 == a.d1);
    //     assert(c.d2 == b.d1);
    //     assert(c.d3 == 1);
    //     assert(c.d4 == 1);
    //     Tensor2D<T> bTranspose(b.d2, b.d1);
    //     for(int i = 0; i < b.d1; ++i)
    //         for(int j = 0; j < b.d2; ++j)
    //             bTranspose(j, i) = b(i, j);
    //     matmul(a, bTranspose, c);
    // }

    void conv2D(u64 fh, u64 fw, u64 padding, u64 stride, u64 ci, u64 co, const Tensor4D<T> &input, const Tensor2D<T> &filter, Tensor4D<T> &output)
    {
        assert(input.d4 == ci);
        assert(filter.d1 == co);
        assert(filter.d2 == fh * fw * ci);
        u64 newH = (((input.d2 + 2*padding - fh)/stride) + 1);
        u64 newW = (((input.d3 + 2*padding - fw)/stride) + 1);
        assert(output.d1 == input.d1);
        assert(output.d2 == newH);
        assert(output.d3 == newW);
        assert(output.d4 == co);

        Conv2DWrapper(input.d1, input.d2, input.d3, input.d4, fh, fw, co, 
            padding, padding, padding, padding, stride, stride, 
            input.data, input.data, filter.data, filter.data, output.data, output.data);
    }


    void div(const Tensor4D<T> &in, T divisor, u64 scale) {
        if (!(divisor & (divisor - 1))) {
            Backend<T>::truncate(in, log2(divisor), 0);
        }
        else {
            T divfp = (1LL << scale) / divisor;
            u64 sz = in.d1 * in.d2 * in.d3 * in.d4;
            for (u64 i = 0; i < sz; i++) {
                in.data[i] *= divfp;
            }
            Backend<T>::truncate(in, scale, 0);
        }
    }

    void divPartial(const Tensor4D<T> &in, T divisor, u64 scale) {
        T divfp = (1LL << scale) / divisor;
        u64 sz = in.d1 * in.d2 * in.d3 * in.d4;
        for (u64 i = 0; i < sz; i++) {
            in.data[i] *= divfp;
        }
    }


    u64 log2(u64 x) {
        u64 y = 0;
        while (x >>= 1) y++;
        return y;
    }

    void select(const Tensor4D<T> &in, const Tensor4D<T> &drelu, const Tensor4D<T> &out) {
        assert(in.d1 == out.d1);
        assert(in.d2 == out.d2);
        assert(in.d3 == out.d3);
        assert(in.d4 == out.d4);
        assert(in.d1 == drelu.d1);
        assert(in.d2 == drelu.d2);
        assert(in.d3 == drelu.d3);
        assert(in.d4 == drelu.d4);
        Select(in.d1 * in.d2 * in.d3 * in.d4, drelu.data, in.data, out.data);
    }

};