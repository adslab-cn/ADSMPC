...
// 文件缓冲区（继承KeyBuf）：支持大文件分块读写
class FileBuf : public KeyBuf
{
public:
    std::fstream file; //读写文件，通过std::fstream操作
...
public:
    ...
    SocketBuf(std::string ip, int port, bool onlyRecv); // 构造时建立TCP连接
